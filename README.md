...
## Alpha8 hardware-test candidate

Alpha8 keeps the Alpha7 policy but makes Tensor G6 / Pixel 11 Thermal materialization sparse: only validated Thermal JSON files whose generated bytes differ from cached stock are placed in the module overlay. Untouched Pixel 11 include-graph files stay on the stock vendor path, and full Stock still creates no Thermal JSON overlay. Pixel 9/10-family targets keep their existing full three-file overlay contract.

Alpha8 is not promoted publicly until the exact candidate passes the Pixel 11 Pro install, reboot, Bootguard/readiness, recovery-state, Outdoor Safe to Stock, WebUI/Action, and quick performance regression gate. See [Alpha8 candidate notes](release-notes/2.1.0-alpha.8.md) and [the vNext device validation matrix](docs/vnext-device-test-matrix.md).
