...
> [!IMPORTANT]
> **2.0.4 remains the current stable release. 2.1.0-alpha.9 is the current public prerelease.** Stable users can remain on 2.0.4. Alpha9 adds the hardware-accepted Tensor G6 sparse-overlay path and preserves untouched Pixel 11 Thermal files byte-for-byte while retaining the Alpha7 recovery controls.
>
...
| Prerelease | `2.1.0-alpha.9` | Pixel 11 sparse Thermal overlays with byte-exact untouched-file preservation, recovery controls, stock-only G6 timing, HUP-safe WebUI and existing Pixel 9 / 9a / 10a vNext support |
...
See [the vNext device validation matrix](docs/vnext-device-test-matrix.md) for the hardware and safety envelope.
