...
The current **test-only G6 recovery candidate** branches the installer and Thermal materializer by device family. Pixel 9/10-family behavior remains on the existing option/patch path. Pixel 11-family installs receive a separate option set with guarded **HotHysteresis + MaxReleaseStep**, **Passive Polling**, Thermal Profile (still capped at Outdoor Safe `+1 C`), the existing ZRAM modes, logging and support snapshot controls. Test 1 defaults to HotHysteresis + MaxReleaseStep `mod`, PassiveDelay `stock 7000`, classic `PollingDelay` stock, Thermal thresholds stock and ZRAM disabled so Harish's benchmark can isolate recovery behavior. The recovery patch is restricted to `thermal_info_config_common.json`: seven named VIRTUAL-SKIN performance sensors, 15 admitted hysteresis-slot changes and 32 admitted `MaxReleaseStep 1 -> 2` cooling-device/profile binding changes across five target sensors. `VIRTUAL-SKIN-SOC-EXTREME`, modem/RF/charging/emergency objects and all other fields remain stock. PassiveDelay `7000 -> 5000` is implemented behind the separate Pixel 11 option for the later second-stage test but is not the Test-1 default.
