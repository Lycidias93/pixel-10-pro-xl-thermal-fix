...
required=(
  ...
  bin/module-control bin/webui-server-arm64 webroot/index.html webroot/app.js webroot/app.css webroot/race-guard.js webroot/race-guard.css webroot/v03.js
...
[[ "$entry_count" -le 90 ]] || { printf 'FAIL release_entry_count_too_high entries=%s max=90\n' "$entry_count"; exit 6; }
...
printf 'PASS release_entry_budget entries=%s max=90\n' "$entry_count"
