...
Installer follow-up from Harish on `grizzly` build `CD1A.260905.001.B1`: the first stock-no-overlay candidate correctly validated the stock Thermal graph and left only `fstab.zram.100p` under `system/vendor/etc`, but `customize.sh` then aborted because it still required every Thermal layout file to exist. The installer now propagates the validator materialization mode into its final active-layout check: `overlay` requires every validated Thermal file, while `stock-no-overlay` requires those generated Thermal files to be absent and permits unrelated files such as `fstab.zram.100p` to keep the directory present. The installer path also accepts the canonical four Recovery modes directly, with legacy `mod` normalized to `combined`.
