...
| Pixel 11 Pro | `grizzly` | 17 | `runtime_verified` baseline + `test_candidate_pending` family recovery/WebUI tuning | bounded Include graph from `thermal_info_config.json` | Harish stock archive established the 10-file Thermal graph plus standalone `thermal_powerbudgets.json` and 35/35 stock `PollingDelay=300000` values. Exact PR #194 baseline candidate SHA-256 `461b150d6ebfc59dbb905fb0f29070c010a938a4b23dd490210e65a7ef83ff3f` passed on `CD1A.260714.001.A9`: module active, Bootguard `full_pass`, readiness `runtime_verified`, 35/35 classic `PollingDelay=300000` preserved and 0 `5000`. Harish later found `PassiveDelay 7000 -> 5000` caused severe prime-core/single-core regression; with recovery tuning retained and PassiveDelay Stock he reported about 2.3k single / 7k multi in Geekbench 7. The current replacement candidate keeps both polling/timing fields Stock and exposes independent HotHysteresis and MaxReleaseStep recovery toggles; their four effective states are Stock, HotHysteresis-only (15 admitted slot changes), MaxReleaseStep-only (32 admitted binding changes), and both enabled (derived internal `combined`, 15+32); family-specific install/Action/WebUI controls and config-first page-cluster selection are aligned. | Classic `PollingDelay`: **Stock only**. `PassiveDelay`: **Stock only / no control**. | Outdoor Safe **+1 C max**, exact master `VIRTUAL-SKIN` only; real-layout regression proves CPU/SOC derivatives and WLAN/BT remain unchanged | blocked on experimental target | Install one exact-head replacement candidate and run the same-device four-state matrix: Stock baseline -> HotHysteresis only -> MaxReleaseStep only -> both toggles enabled, with Thermal Stock + ZRAM disabled/page-cluster Stock throughout. For each mode verify exact recovery-field inventory, Bootguard/readiness/overlay, unchanged PollingDelay/PassiveDelay and benchmark/recovery behavior; then verify WebUI/Action/manager surfaces and Thermal Stock -> +1 -> Stock changes only exact `VIRTUAL-SKIN` before any clean integration merge |
...
Alpha8 narrows Tensor G6 module materialization without widening the Thermal policy: only Pixel 11 Thermal JSON files with an actual admitted policy mutation are placed under the module overlay. Unmodified graph members are preserved byte-for-byte, including source files without a trailing newline, so AWK stream reconstruction cannot create false sparse-overlay diffs. Outdoor Safe is file-scoped to the master `VIRTUAL-SKIN` definition in `thermal_info_config_common.json`; same-named entries in stats, forecast, background-task, or other peripheral graph files remain stock. Full Stock continues to use `stock-no-overlay`. Pixel 9/10-family targets remain on the established full three-file `overlay` contract.
...
1. classic `PollingDelay` remains mandatory Stock on Pixel 11; the existing global `300000 -> 5000` path stays blocked. `PassiveDelay` tuning has been removed after real-device performance regression and remains Stock-only. Family-local Recovery exposes independent HotHysteresis and MaxReleaseStep toggles; benchmark Stock, each toggle in isolation, and the derived both-enabled state;
2. Outdoor Safe is capped at `+1 C` and may change only the exact master `VIRTUAL-SKIN` sensor in the real G6 `thermal_info_config_common.json` layout; CPU/SOC derivative sensors, WLAN/BT/MMW, modem, charge and protection objects are explicit non-target regression guards;
...
The current replacement candidate also synchronizes the Pixel 11 installer, Action dashboard, standalone WebUI and manager-card markers around one family contract. ZRAM page-cluster becomes a persisted install/runtime sub-choice but remains post-Bootguard gated. The standalone capabilities payload deliberately stays on the pinned generic v1 schema (no top-level `device_family`) while runtime/status carries family context; this avoids the embedded-host startup failure observed in the earlier candidate.
