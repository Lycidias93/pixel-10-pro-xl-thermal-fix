...
| Pixel 11 Pro | `grizzly` | 17 | `runtime_verified` baseline + `test_candidate_pending` family recovery/WebUI tuning | bounded Include graph from `thermal_info_config.json` | Harish stock archive established the 10-file Thermal graph plus standalone `thermal_powerbudgets.json` and 35/35 stock `PollingDelay=300000` values. Exact PR #194 baseline candidate SHA-256 `461b150d6ebfc59dbb905fb0f29070c010a938a4b23dd490210e65a7ef83ff3f` passed on `CD1A.260714.001.A9`: module active, Bootguard `full_pass`, readiness `runtime_verified`, 35/35 classic `PollingDelay=300000` preserved and 0 `5000`. Harish later found `PassiveDelay 7000 -> 5000` caused severe prime-core/single-core regression; with recovery tuning retained and PassiveDelay Stock he reported about 2.3k single / 7k multi in Geekbench 7. The replacement candidate keeps both polling/timing fields Stock, adds 15 exact HotHysteresis + 32 MaxReleaseStep binding changes, family-specific install/Action/WebUI controls and config-first page-cluster selection. | Classic `PollingDelay`: **Stock only**. `PassiveDelay`: **Stock only / no control**. | Outdoor Safe **+1 C max**, exact master `VIRTUAL-SKIN` only; real-layout regression proves CPU/SOC derivatives and WLAN/BT remain unchanged | blocked on experimental target | Install exact replacement candidate with Recovery Mod + Thermal Stock + ZRAM disabled; verify standalone WebUI starts, family markers are correct, benchmark recovery, then exercise Thermal Stock -> +1 -> Stock and prove only exact `VIRTUAL-SKIN` changes before any clean integration merge |
...
1. classic `PollingDelay` remains mandatory Stock on Pixel 11; the existing global `300000 -> 5000` path stays blocked. `PassiveDelay` tuning has been removed after real-device performance regression and remains Stock-only. Family-local HotHysteresis + MaxReleaseStep recovery tuning is the separate admitted test surface;
