...
1. classic `PollingDelay` remains mandatory Stock on Pixel 11; the existing global `300000 -> 5000` path stays blocked. `PassiveDelay` tuning has been removed after real-device performance regression and remains Stock-only. Family-local Recovery is a four-mode admitted test surface: Stock / HotHysteresis / MaxReleaseStep / Combined; the isolated modes must be benchmarked separately before Combined;
