...
The integrated Alpha7 candidate subsequently completed the requested `grizzly` hardware matrix: Stock + ZRAM remained stock-no-overlay, HotHysteresis and MaxReleaseStep worked independently and together, WebUI/Action state stayed aligned, and the quick regression check passed. PR #199 was then integrated into the Alpha7 line without widening the Stock-only polling/timing policy.

The accepted Alpha7 candidate synchronizes the Pixel 11 installer, Action dashboard, standalone WebUI and manager-card markers around one family contract. ZRAM page-cluster becomes a persisted install/runtime sub-choice but remains post-Bootguard gated. The standalone capabilities payload deliberately stays on the pinned generic v1 schema (no top-level `device_family`) while runtime/status carries family context; this avoids the embedded-host startup failure observed in the earlier candidate.
