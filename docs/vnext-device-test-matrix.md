...
| Pixel 11 Pro | `grizzly` | 17 | `runtime_verified` baseline + `test_candidate_pending` recovery tuning | bounded Include graph from `thermal_info_config.json` | Harish stock archive established the 10-file Thermal graph plus standalone `thermal_powerbudgets.json` and 35/35 stock `PollingDelay=300000` values. Exact PR #194 baseline candidate SHA-256 `461b150d6ebfc59dbb905fb0f29070c010a938a4b23dd490210e65a7ef83ff3f` passed on `CD1A.260714.001.A9`: module active, Bootguard `full_pass`, readiness `runtime_verified`, 35/35 classic `PollingDelay=300000` preserved and 0 `5000`. The new test-only family candidate keeps that classic polling baseline while adding exact `thermal_info_config_common.json` HotHysteresis + MaxReleaseStep recovery controls. | Classic `PollingDelay`: **Stock only**. Test 1 PassiveDelay: **Stock 7000**; phase-2 5000 path exists but requires test-1 evidence first. | +1 C cap remains available, but Test 1 default is Stock thresholds | blocked on experimental target | Install exact test-1 candidate with recovery mod + PassiveDelay stock + Thermal stock + ZRAM disabled; benchmark throttle/recovery behavior and return post-reboot/runtime evidence before any integration merge or phase-2 PassiveDelay promotion |
...
1. classic `PollingDelay` remains mandatory Stock on Pixel 11; the existing global `300000 -> 5000` path stays blocked. Family-local recovery tuning is a separate test surface. Test 1 keeps `PassiveDelay=7000`; the guarded `7000 -> 5000` PassiveDelay mode is phase-2 only and requires fresh exact-head `grizzly` evidence before integration;
