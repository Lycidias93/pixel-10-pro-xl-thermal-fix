...
- **WebUI previews/dry-runs stay non-mutating on Android.** Boolean request parsing no longer relies on GNU-only `sed` behavior, preventing a preview such as `dry_run=true` from falling through to the productive action path.
