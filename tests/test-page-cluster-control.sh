...
set +e
env ZRAM_CONFIG_FILE="$TMP/config.env" PAGE_CLUSTER_STATE_DIR="$TMP/state" PAGE_CLUSTER_PATH="$TMP/page-cluster" PAGE_CLUSTER_SWAPS_FILE="$TMP/swaps" PAGE_CLUSTER_BOOT_ID_FILE="$TMP/boot_id" sh "$HELPER" apply-zero > "$TMP/non-zram.log" 2>&1
rc=$?
set -e
test "$rc" = 3 || { echo "FAIL: expected no-active-zram rc=3 actual=$rc" >&2; exit 1; }
...
set +e
env ZRAM_CONFIG_FILE="$TMP/config.env" PAGE_CLUSTER_STATE_DIR="$TMP/state" PAGE_CLUSTER_PATH="$TMP/page-cluster" PAGE_CLUSTER_SWAPS_FILE="$TMP/swaps" PAGE_CLUSTER_BOOT_ID_FILE="$TMP/boot_id" sh "$HELPER" apply-zero > "$TMP/zram-disabled.log" 2>&1
rc=$?
set -e
test "$rc" = 2 || { echo "FAIL: expected zram-disabled rc=2 actual=$rc" >&2; exit 1; }
grep -Fq 'RESULT: PAGE_CLUSTER_ZERO_BLOCKED reason=zram_not_explicitly_enabled' "$TMP/zram-disabled.log"

printf 'ENABLE_ZRAM_100P=1\nZRAM_RISK_ACK=explicit_user_enable\nZRAM_PAGE_CLUSTER_MODE=zero\nZRAM_PAGE_CLUSTER_RISK_ACK=explicit_user_zero\n' > "$TMP/config.env"
printf 'invalid' > "$TMP/page-cluster"
set +e
env ZRAM_CONFIG_FILE="$TMP/config.env" PAGE_CLUSTER_STATE_DIR="$TMP/state" PAGE_CLUSTER_PATH="$TMP/page-cluster" PAGE_CLUSTER_SWAPS_FILE="$TMP/swaps" PAGE_CLUSTER_BOOT_ID_FILE="$TMP/boot_id" PAGE_CLUSTER_CALLER=test_reconcile_fail sh "$HELPER" reconcile > "$TMP/reconcile-fail.log" 2>&1
rc=$?
set -e
test "$rc" = 5 || { echo "FAIL: expected reconcile failure rc=5 actual=$rc" >&2; cat "$TMP/reconcile-fail.log" >&2; exit 1; }
grep -Fq 'RESULT: PAGE_CLUSTER_ZERO_BLOCKED reason=invalid_baseline' "$TMP/reconcile-fail.log"
grep -Fq 'RESULT: PAGE_CLUSTER_RECONCILE_FAIL command_exit_code=5' "$TMP/reconcile-fail.log"
