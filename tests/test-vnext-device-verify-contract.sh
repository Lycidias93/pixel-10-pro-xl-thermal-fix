...
grep -Fq 'verifier_version=v2' "$verify"
grep -Fq 'baseline|post-disable|post-reenable' "$verify"
grep -Fq 'root_transition=pass' "$verify"
grep -Fq 'command -v su' "$verify"
...
grep -Fq '2.1.0-alpha.6|2.1.0-alpha.5|2.1.0-alpha.4|2.1.0-alpha.4-dev.2) pass_check vnext_supported_version' "$verify"
...
printf '%s\n' 'PASS alpha6_current_and_previous_bound_identity'
