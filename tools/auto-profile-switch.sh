...
case "$THERMAL_OUTDOOR_PROFILE" in
  outdoor-safe|outdoor-plus|outdoor-extended)
    ...
    if [ -s "$OUTDOOR_PROFILE_DIR/thermal_info_config_throttling.json" ] && [ -s "$OUTDOOR_PROFILE_DIR/thermal_info_config.json" ] && [ -s "$OUTDOOR_PROFILE_DIR/thermal_info_config_charge.json" ]; then
      ...
      case "$THERMAL_OUTDOOR_PROFILE" in
        outdoor-safe)
          PROFILE_STATE="${PROFILE_STATE}_outdoor_safe_test18"
          BUILD_STATE="${BUILD_STATE}_outdoor_safe_test18"
        ;;
        outdoor-plus)
          PROFILE_STATE="${PROFILE_STATE}_outdoor_plus_test18"
          BUILD_STATE="${BUILD_STATE}_outdoor_plus_test18"
        ;;
        outdoor-extended)
          PROFILE_STATE="${PROFILE_STATE}_outdoor_extended_test18"
          BUILD_STATE="${BUILD_STATE}_outdoor_extended_test18"
        ;;
      esac
    else
