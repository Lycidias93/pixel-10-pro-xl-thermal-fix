...
collect_file meta_backend_probe.txt sh -c 'echo "== su version =="; su -v 2>/dev/null || true; su -V 2>/dev/null || true; echo; echo "== backend files =="; find /data/adb /debug_ramdisk /sbin -maxdepth 5 \( -iname "*mountify*" -o -iname "*metamodule*" -o -iname "*meta-module*" \) 2>/dev/null | sort | head -80; echo; echo "== module props meta/mountify =="; for p in /data/adb/modules/*/module.prop /data/adb/modules_update/*/module.prop; do [ -f "$p" ] || continue; if grep -Eiq "mountify|metamodule|meta module|meta-module" "$p"; then echo "-- $p"; grep -E "^(id=|name=|version=|versionCode=|description=)" "$p"; fi; done'
...
collect_file mountinfo_thermal.txt sh -c 'echo "== overlay hash equivalence =="; for f in thermal_info_config_throttling.json thermal_info_config.json thermal_info_config_charge.json; do active="$(sha256sum "/vendor/etc/$f" 2>/dev/null | awk "{print \$1}")"; overlay="$(sha256sum "$0/system/vendor/etc/$f" 2>/dev/null | awk "{print \$1}")"; orig="$(sha256sum "/data/adb/pixel-10-pro-xl-thermal-fix/originals/$f" 2>/dev/null | awk "{print \$1}")"; echo "file=$f active=$active overlay=$overlay backup=$orig"; done; echo; echo "== raw thermal mountinfo =="; grep -E "thermal_info_config(_charge|_throttling)?\.json" /proc/self/mountinfo || true' "$MODDIR"
...
# PIXEL_THERMAL_BOOT_CRASH_CONTEXT_COLLECT_START
collect_file boot_crash_context.txt sh -c 'echo "== module paths =="; for d in /data/adb/modules/pixel-10-pro-xl-thermal-fix /data/adb/modules_update/pixel-10-pro-xl-thermal-fix; do echo "-- $d"; ls -la "$d" 2>/dev/null || true; [ -s "$d/module.prop" ] && grep -E "^(id|name|version|versionCode|description|updateJson)=" "$d/module.prop" || true; [ -e "$d/disable" ] && echo disable=present || echo disable=absent; done; echo; echo "== pstore =="; ls -la /sys/fs/pstore 2>/dev/null || true; echo; echo "== crash buffer =="; logcat -d -b crash -t 800 2>/dev/null || true; echo; echo "== boot crash collector =="; [ -x "$0/tools/boot-crash-log-collect.sh" ] && echo boot_crash_collector=present || echo boot_crash_collector=absent' "$MODDIR"
# PIXEL_THERMAL_BOOT_CRASH_CONTEXT_COLLECT_END
