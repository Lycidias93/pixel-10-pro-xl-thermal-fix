...
collect_file thermal_tombstone_index.txt sh -c 'for f in /data/tombstones/tombstone_*; do [ -f "$f" ] || continue; if grep -a -E -m1 "thermal|ThermalHAL|android.hardware.thermal" "$f" >/dev/null 2>&1; then ls -l "$f"; grep -a -E -m3 "Cmdline:|pid:|thermal|ThermalHAL|android.hardware.thermal" "$f" || true; echo; fi; done'
