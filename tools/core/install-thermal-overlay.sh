...
thermal_install_overlay() {
  ...
  if [ -s "$MODPATH/tools/menu/thermal-outdoor-menu.sh" ]; then
    ...
    BASE_PROFILE="dynamic" MODDIR="$MODPATH" sh "$MODPATH/tools/menu/thermal-outdoor-menu.sh" install || ui_print "! Thermal outdoor menu failed nonfatal; keeping stock profile"
  else
  ...
  if [ -s "$MODPATH/tools/core/patch-thermal.sh" ]; then
    ...
    sh "$MODPATH/tools/core/patch-thermal.sh" "$THERMAL_POLLING_MODE" "$THERMAL_OUTDOOR_PROFILE" "$MODPATH" || thermal_abort "! Dynamic patching failed"
  else
