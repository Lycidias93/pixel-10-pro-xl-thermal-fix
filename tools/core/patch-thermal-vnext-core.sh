#!/system/bin/sh
# Experimental vNext materializer for Pixel 9-series / Pixel 10a layouts.
set -eu
...
normalize_allowed() {
  ...
  awk '
    ...
    function mask_numbers(text, out) {
      out=""
      while (match(text, /[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)/)) { out=out substr(text,1,RSTART-1) "__HOT_VALUE__"; text=substr(text,RSTART+RLENGTH) }
      return out text
    }
    BEGIN { target=0; in_hot=0 }
    {
      line=$0
      if (!in_hot) {
        if ((line ~ /"Name"[[:space:]]*:[[:space:]]*"VIRTUAL-SKIN/ || line ~ /"Name"[[:space:]]*:[[:space:]]*"cellular-emergency/) && line !~ /OVER-35C/) target=1
        else if (line ~ /"Name"[[:space:]]*:/) target=0
      }
...
patch_one() {
  ...
  awk -v delta="$DELTA" -v poll_mode="$POLLING_MODE" '
    function adjust_numbers(text, out, tok, n, dot, dec, fmt, v) {
...
if [ "$cache_valid" -eq 1 ]; then
  ...
  while IFS="$_tab" read -r file expected_sha expected_bytes poll300000 extra; do
    ...
    case " $THERMAL_LAYOUT_FILES " in *" $file "*) ;; *) cache_valid=0; continue ;; esac
  ...
  [ "$_rows" -eq 3 ] 2>/dev/null || cache_valid=0
fi
...
if [ "$cache_valid" -ne 1 ]; then
  ...
  for file in $THERMAL_LAYOUT_FILES; do
    ...
    if [ "$SOURCE_DIR" = /vendor/etc ] || [ "$SOURCE_DIR" = /system/vendor/etc ]; then mf="$TARGET_DIR/$file"; if [ -s "$mf" ] && [ "$(sha_file "$sf")" = "$(sha_file "$mf")" ]; then fail 32 "active_overlay_detected_without_valid_cache_$file"; fi; fi
    ...
    [ "$p5" = 0 ] || fail 34 "patched_source_5000_rejected_$file"; [ "$p30" = 0 ] || fail 35 "source_30000_rejected_$file"; [ "$low" = 0 ] || fail 36 "source_lowercase_polling_rejected_$file"
  ...
  [ "$source_files" -eq 3 ] || fail 37 "source_inventory_invalid_$source_files"
  ...
  mv "$CACHE_STAGE" "$CACHE_DIR" || fail 39 cache_promotion_failed; CACHE_PROMOTED=1; rm -rf "$CACHE_OLD"
else
  thermal_layout_from_manifest "$MANIFEST" || fail 29 cached_layout_invalid
fi

rm -rf "$PATCH_STAGE" "$TARGET_OLD"; mkdir -p "$PATCH_STAGE"
if [ -d "$TARGET_DIR" ]; then for existing in "$TARGET_DIR"/*; do [ -e "$existing" ] || continue; name="${existing##*/}"; case "$name" in thermal_info_config*.json) continue ;; esac; cp -fpR "$existing" "$PATCH_STAGE/"; done; fi
...
while IFS="$_tab" read -r file source_sha source_bytes source_polling; do
  [ "$file" = file ] && continue; [ -n "$file" ] || continue
  sf="$CACHE_DIR/$file"; of="$PATCH_STAGE/$file"; patch_one "$sf" "$of"; thermal_json_tolerant_validate "$of" || fail 40 "output_structure_invalid_$file"
  ...
  [ "$o30" = 0 ] || fail 41 "output_30000_rejected_$file"; [ "$olow" = 0 ] || fail 42 "output_lowercase_polling_rejected_$file"
  if [ "$POLLING_MODE" = mod ]; then [ "$o300" = 0 ] || fail 43 "remaining_300000_$file"; [ "$o5" = "$source_polling" ] || fail 44 "replacement_count_${file}_${o5}_expected_${source_polling}"; replacements="$source_polling"; else [ "$o300" = "$source_polling" ] || fail 45 "stock_300000_changed_$file"; [ "$o5" = 0 ] || fail 46 "stock_contains_5000_$file"; replacements=0; fi
  ns="$GUARD_DIR/.norm-source.$$"; no="$GUARD_DIR/.norm-output.$$"; normalize_allowed "$sf" "$ns"; normalize_allowed "$of" "$no"; cmp -s "$ns" "$no" || fail 47 "unallowed_byte_change_$file"; rm -f "$ns" "$no"
  ...
  printf '    "%s": {\n' "$file" >> "$REPORT_TMP"; printf '      "source_sha256": "%s",\n' "$source_sha" >> "$REPORT_TMP"; printf '      "output_sha256": "%s",\n' "$osh" >> "$REPORT_TMP"; printf '      "source_polling_300000": %s,\n' "$source_polling" >> "$REPORT_TMP"; printf '      "replacements": %s,\n' "$replacements" >> "$REPORT_TMP"; printf '      "output_polling_300000": %s,\n' "$o300" >> "$REPORT_TMP"; printf '      "output_polling_5000": %s,\n' "$o5" >> "$REPORT_TMP"; printf '%s\n' '      "validation": "passed"' '    }' >> "$REPORT_TMP"
...
[ "$source_files" -eq 3 ] || fail 48 output_layout_count_invalid
for file in $THERMAL_LAYOUT_FILES; do [ -s "$PATCH_STAGE/$file" ] || fail 48 "required_output_missing_$file"; done

printf '%s\n' '  },' '  "totals": {' >> "$REPORT_TMP"; printf '    "source_files": %s,\n' "$source_files" >> "$REPORT_TMP"; printf '    "source_polling_300000": %s,\n' "$source_polling_total" >> "$REPORT_TMP"; printf '    "replacements": %s,\n' "$replacement_total" >> "$REPORT_TMP"; printf '    "output_polling_300000": %s,\n' "$output_300000_total" >> "$REPORT_TMP"; printf '    "output_polling_5000": %s\n' "$output_5000_total" >> "$REPORT_TMP"; printf '%s\n' '  },' '  "validation": "passed"' '}' >> "$REPORT_TMP"
thermal_json_tolerant_validate "$REPORT_TMP" || fail 49 validation_report_invalid
if [ "$POLLING_MODE" = mod ]; then [ "$replacement_total" = "$source_polling_total" ] || fail 50 total_replacement_mismatch; [ "$output_300000_total" = 0 ] || fail 51 total_remaining_300000; [ "$output_5000_total" = "$source_polling_total" ] || fail 52 total_5000_mismatch; else [ "$replacement_total" = 0 ] || fail 53 stock_replacements_nonzero; [ "$output_300000_total" = "$source_polling_total" ] || fail 54 stock_total_300000_changed; [ "$output_5000_total" = 0 ] || fail 55 stock_total_5000_nonzero; fi

if [ -d "$TARGET_DIR" ]; then mv "$TARGET_DIR" "$TARGET_OLD"; fi
mv "$PATCH_STAGE" "$TARGET_DIR" || fail 56 target_atomic_promotion_failed; PROMOTED=1; rm -rf "$TARGET_OLD"
mv "$PATCH_MANIFEST_TMP" "$PATCH_MANIFEST"; mv "$REPORT_TMP" "$REPORT_MODULE"; cp -fp "$REPORT_MODULE" "$REPORT_DATA"
