...
ZIP="$DL/pixel_thermal_outdoor_boot_debug_$TS.zip"
TGZ="$DL/pixel_thermal_outdoor_boot_debug_$TS.tar.gz"
...
# Android logcat -L reads the previous-boot pstore log when supported.
...
for _dir in /sys/fs/pstore /proc; do
  [ -d "$_dir" ] || continue
  case "$_dir" in
    /sys/fs/pstore)
      find "$_dir" -maxdepth 1 -type f -print 2>/dev/null | while IFS= read -r _file; do
