...
if [ -r "$MODULE_PROP" ]; then
  ...
  case "$module_version" in 2.1.0-alpha.4|2.1.0-alpha.4-dev.2) pass_check alpha4_version ;; *) fail_check "unexpected_module_version=${module_version:-missing}" ;; esac
else
  EVIDENCE=partial
...
if [ -s "$COLLECTOR" ]; then
  ...
  MODDIR="$MODDIR" /system/bin/sh "$COLLECTOR" "device_verify_$PHASE" "$thermal_arg" unknown alpha4_device_test > "$collector_log" 2>&1
  collector_rc=$?
