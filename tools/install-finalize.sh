...
thermal_finalize_install() {
...

  {
    ...
    printf '%s\n' "profile_source_channel=${THERMAL_PROFILE_CHANNEL:-unknown}"
    printf '%s\n' "profile_source_family=${THERMAL_PROFILE_FAMILY:-unknown}"
    printf '%s\n' "profile_source_path=${THERMAL_PROFILE_REL:-unknown}"
    printf '%s\n' "profile_bundle_sha256=${THERMAL_PROFILE_BUNDLE_SHA256:-unknown}"
    ...
    printf '%s\n' "overlay_materializer=exact_git_profile_atomic_v2"
    ...
    printf '%s\n' ""
    printf '%s\n' "expected_thermal_files=${THERMAL_PROFILE_JSON_COUNT:-unknown}"
    printf '%s\n' "polling_values_changed_by_this_release=exact_existing_PollingDelay_300000_to_5000_only"
    printf '%s\n' "bind_mount_model=no"
