...
## Test 3 mobile UI cleanup

- Supersedes `v1.5.1-universal-test.2` for UI validation.
- Renames Action menu `Cycle` to `Settings`.
- Shortens installer and Action text for mobile Magisk/KSU views.
- Keeps META-INF packaging.
- Keeps the ZRAM Ampel false-negative fix from test2.
- Stable `update.json` remains on `1.5-universal.1`.

## Mobile UI polish

- Replaces confusing `Cycle` label with `Settings`.
- Makes Action status output compact for phone screens.
- Keeps ZRAM runtime false-negative fix and META-INF packaging.
