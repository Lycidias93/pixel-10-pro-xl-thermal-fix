...
variant_exists() {
  _variant="$1"
  case "$_variant" in stock|base) return 0 ;; esac
  if command -v profile_matrix_variant >/dev/null 2>&1; then
    _path="$(profile_matrix_variant "$BASE_PROFILE" "$_variant" 2>/dev/null || true)"
    [ -n "$_path" ] && [ -s "$MODDIR/profiles/$_path/system/vendor/etc/thermal_info_config_throttling.json" ] && return 0
  fi
...
if [ "$(cfg_get THERMAL_SETTINGS_MODE)" = "last" ]; then
  ...
  msg ""; msg "Use last settings:"; msg "$choice_label"; msg "----------------------------------------"
  case "$choice" in
